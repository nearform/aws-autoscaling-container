...
module.exports = function (config, logger) {
  ...
  function allowSnsToPublishOnSqs (autoScalingGroupName, sqsQueueUrl, sqsQueueArn, snsTopicArn, cb) {
    var policy = JSON.stringify({
      Version: "2012-10-17",
      Statement: [{
        Sid: autoScalingGroupName,
        Effect: "Allow",
        Principal: "*",
  ...
  function setupNotifications (autoScalingGroupName, cb) {
      async.waterfall([
        function snsTopic (cb) {
          createSnsTopic(autoScalingGroupName, function  (err, data) {
            if (err) {
              return cb(err);
            }
        ...
        function sqsQueue (notificationData, cb) {
          createSqsQueue(autoScalingGroupName, function (err, data) {
            if (err) {
              return cb(err);
            }
